...
<html lang="en">
  <head>
    <title>Služebníček 2.D | Přidávání záznamu</title>
  ...
  <div class="input">
    <form method="POST">
      ...
      Skupina:
      <label>
      <input type="checkbox" name="skupina1" name="skupina_1" <?= (@$newTask['skupina1'] == 'on' || empty($newTask)) ? 'checked' : '' ?>>
      1
      </label>
      <label>
      <input type="checkbox" name="skupina2" name="skupina_2" <?= (@$newTask['skupina2'] == 'on' || empty($newTask)) ? 'checked' : '' ?>>
      2
      </label>
      <br>
      <span>Popis:</span>
      <input type="text" name="popis" placeholder="př: UNPH 97/8 a,b,c" value="<?= @$newTask['popis'] ?>" required>
      <br>
